...
class TicTacToeBoard:
    ...
    def __init__(self, root, board_size: int, consecutive_symbols: int, game_instance, game_board) -> None:
        self.root = root
    ...
    def initialize_board(self):
        ...
        self.consecutive_symbols = self.game_instance.consecutive_symbols
...
class Game:
    ...
    def show_game_board(self):
        ...
        if self.board_size is not None and self.consecutive_symbols is not None:
            board_root = tk.Tk()
            board_root.title("Tic Tac Toe Board")
            self.tic_tac_toe_board = TicTacToeBoard(board_root, self.board_size, self.consecutive_symbols, self, self.board)
            self.tic_tac_toe_board.board_size = self.board_size
            self.tic_tac_toe_board.consecutive_symbols = self.consecutive_symbols
            board_root.mainloop()
        else:
    ...
    def get_move(self, row, col):
        ...
        if self.is_valid_move(row, col):
            ...
            if self.is_win(current_player.symbol, self.consecutive_symbols):
    ...
    def is_valid_move(self, row: int, col: int) -> bool:
        ...
        if self.board is not None and 0 <= row < len(self.board) and 0 <= col < len(self.board[0]):
            return self.board[row][col] == " "  # Check if the corresponding button is empty
        else:
            return False

    def get_best_move(self, player_symbol: str, opponent_symbol: str, consecutive_symbols: int, max_depth: int) -> tuple:
        ...
        for row, column in empty_spaces:
            ...
            score = self.minimax(self.board, 0, alpha, beta, False, consecutive_symbols, player_symbol, opponent_symbol, max_depth)
    ...
    def minimax(self, board: list, depth: int, alpha: int, beta: int, is_maximizing: bool, consecutive_symbols: int, player_symbol: str, opponent_symbol: str, max_depth: int):
        ...
        if depth == max_depth or self.is_win(player_symbol, consecutive_symbols) or self.is_win(opponent_symbol, consecutive_symbols):
            return self.evaluate(player_symbol, opponent_symbol, consecutive_symbols)
        ...
        if is_maximizing:
            ...
            for i, j in empty_spaces:
                ...
                score = self.minimax(board, depth + 1, alpha, beta, False, consecutive_symbols, player_symbol, opponent_symbol, max_depth)
                board[i][j] = " "
            ...
            for i, j in empty_spaces:
                ...
                score = self.minimax(board, depth + 1, alpha, beta, True, consecutive_symbols, player_symbol, opponent_symbol, max_depth)
                board[i][j] = " "
    ...
    def ai_turn(self) -> None:
        ...
        consecutive_symbols = self.consecutive_symbols
        max_depth = 3

        best_move = self.get_best_move(player_symbol, opponent_symbol, consecutive_symbols, max_depth)
        ...
        if not self.get_empty_spaces() and not self.is_win(player_symbol, consecutive_symbols) and not self.is_win(opponent_symbol, consecutive_symbols):
            print("It's a tie!")
...
def initialize_settings_window(root):
    ...
    win_condition_label = tk.Label(settings_window, text="Enter the desired number of consecutive symbols (should not exceed board size):")
...
def start_game(settings_window, board_size_entry, win_condition_entry, player_var, difficulty_var):
    ...
    tic_tac_toe_game.consecutive_symbols = win_condition
...
def main():
    ...
    initialize_settings_window(root)
